...
# Histogram Insulin
par(mfrow=c(4,2))
ggplot(data, aes(data[,5],fill=..count..)) + 
      geom_histogram(aes(y=..density..)) +
      geom_density(alpha=.2, fill="purple")
```
...
# Latihan
Berdasarkan kasus diatas, cobalah anda lakukan klasifikasi untuk mengidentifikasi pasien yang terkena diabetes dan yang tidak terkena diabetes serta lakukan evaluasi atas model klasifikasi yang sudah dihasilkan. Partisilah data ke dalam data training dan data testing. Gunakan salah satu model machine learning untuk klasifikasi yang sudah dipelajari sebelumnya. SVM/Naive Bayes/XGBoost/ dll.
...
modelForest
```
Dari mode dapat dilihat bahwa tingkat kesalahan adalah 23.98%, dengan akurasi sebesar 76.02%

**Latihan**
Lakukan tuning hyperparameter untuk meningkatkan kinerja dari model.

**Latihan*
Coba dilakukan ekplorasi pada metode Hyperparameter tuning. Ada 3 pendekatan atau metode:
1. Grid Search
2. Random Search
3. Bayesian Search
...
modelForest2 <- randomForest(data=data.train,
               as.factor(Outcome)~.,
               ntree=100, mtry=3)
...
```
```{r}
head(data.test$Outcome)
```
```{r}
prediksiForest2 <- predict(modelForest2, data.test)
head(prediksiForest2, 10)
```
```{r}
...
```
```{r}
conf_Matrix2 <- table(prediksiForest2, data.test$Outcome)
conf_Matrix2
```
...
recall <- confusionMatrix(cm)$byClass["Recall"]
...
```
```{r}
cm2 <-  table(data.test[,9], prediksiForest2)

accuracy2 <- confusionMatrix(cm2)$overall["Accuracy"]
precision2 <- confusionMatrix(cm2)$byClass["Precision"]
recall2 <- confusionMatrix(cm2)$byClass["Recall"]
F1_2 <- confusionMatrix(cm2)$byClass["F1"]
...
- Nilai akurasi Anda adalah 0.773913, yang berarti model Anda berhasil mengklasifikasikan dengan benar sekitar 77.39% dari seluruh instance.
- Nilai precision Anda adalah 0.8486842, yang berarti sekitar 84.86% dari instance yang diklasifikasikan sebagai positif oleh model adalah benar-benar positif.
- Nilai sensitivity(Recall) Anda adalah 0.8164557, yang berarti model Anda berhasil mengidentifikasi sekitar 81.64% dari semua instance yang sebenarnya positif.
- Nilai F1 score Anda adalah 0.8322581, yang menunjukkan keseimbangan yang baik antara precision dan recall.
...
** Latihan **
Lakukan analisis klasifikasi dengan model ML yang lain, seperti SVM.
...
predictSVM <- predict(modelSVM, data.test)
table(predictSVM, data.test$Outcome)
```
...
** Latihan **
Buatlah ringkasan terkait dengan hyperparameter tuning dalam peningkatan kinerja model prediksi. Secara umum, hyperparameter tersebut terbagi atas:
1. Grid Search?
2. Random Search?
3. Bayesian Search?

Jelaskan masing-masing perbedaannya. Kemudian buatkan contoh penerapannya dalam R Code.
