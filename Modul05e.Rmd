...
**Support Vector Machine (SVM)** adalah sebuah algoritma machine learning yang digunakan untuk tugas klasifikasi dan regresi. Untuk prediksi dengan data kontinu, maka dapat diterapkan model SVR (Support Vector Regression).

Ini adalah model prediktif yang efektif dan dapat digunakan baik untuk data terstruktur maupun tidak terstruktur. Berikut adalah penjelasan detil tentang SVM:
...
```{r}
# library untuk manipulasi data, tanggal dan visualsiasi (dplyr, lubridate, ggplot2)
library(tidyverse)
# library untuk membagi partisi data menjadi data training dan testing (bentuk lain dari library caret)
library(caTools)
...
```
Interpretasi:
1. Baris pertama (0) menunjukkan situasi ketika sebenarnya adalah kelas 0. Model berhasil memprediksi dengan benar sebagai kelas 0 sebanyak 1609 kali.
...
2. Baris kedua (1) menunjukkan situasi ketika sebenarnya adalah kelas 1. Model hanya berhasil memprediksi dengan benar sebagai kelas 1 sebanyak 234 kali, sedangkan seharusnya adalah 358 kali.
...
```{r}
accuracy <- (2179+102)/(2179+234+358+102)
accuracy
```
...
$$Recall= TP/(TP + FN)$$
```{r}
recall <- 2179/(2179+358)
recall
```
*Latihan dirumah*
ekplorasi lagi satu matrik lagi namanya adalah F1-Score
