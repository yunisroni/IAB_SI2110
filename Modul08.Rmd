...
summary(lm.r)
```
Nilai P-Value = 0.00000000000000199 < 0.05 (sangat signifikan)

$$Sales = \beta_0 + \beta_1facebook + \varepsilon$$


$$Sales = 11.23253 + 0.21291.facebook + \varepsilon$$ 

Misalnya facebooknya Rp. 3500, berapakah nilai penjualan (sales)?
Persamaan regresi linernya adalah:

$$Sales = 11.23253 + (0.21291*3500) + \varepsilon$$ 

```{r}
sales_f = 11.02329 + (0.20290*3500)
sales_f
```
...
summary(lm.y)
```
nilai p-value = 0.00000000000000022 (p-value < 0,05) -> sangat signifikan


Berdasarkan output diatas bisa dijelaskan bahwa:
Persamaan regresi linernya adalah

$$sales = 8.414531 + 0.047215*youtube$$

Misal youtube = 3500

```{r}
sales.y = 8.414531 + (0.047215*3500)
sales.y
```
...
```
$$Sales = 14.84619 + 0.05993 * newspaper + \varepsilon$$
Misal nilai newspapernya: 3500
maka nilai:

```{r}
sales_n = 14.84619 + (0.05993 * 3500)
sales_n
```
...
model_performance <- data.frame(
  ...
  SSE = c(sse_f, sse_y, sse_n),
  Rquare = c(r_squared_f, r_squared_y, r_squared_n),
