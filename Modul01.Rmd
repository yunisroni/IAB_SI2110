...
```{r}
# character
a_char <- c("Universitas", "Mikroskil", "Sistem Informasi", "Medan", "Roni Yunis")
# complex
...
```{r}
orang <- c("Katanya", "Fufufafa", "Bohong")
```
...
```{r}
campur <- c("Aisyah", 2020, 46L, TRUE, 3*2i)
campur
```
...
```{r}
nilai <- matrix(11:22, nrow = 3, ncol = 4)
nilai
...
*latihan:* 
coba akses `our_list` pada index ke 2 dan cek tipe datanya menggunakan fungsi `class()`
...
```
*Latihan*
buatlah sebuah list yang berisikan nilai sebagai berikut:

- c("Universitas","Mikroskil", "Umurnya", 27, "Tahun", 20212, c(TRUE,FALSE))

simpan ke dalam objek dengan nama: mikroskil

```{r}
mikroskil <- list(c("Universitas","Mikroskil"), "Umurnya", 27, "Tahun", 20212, c(TRUE,FALSE))
...
buatlah data frame dari daftar nilai/KHS anda:
Kode, Nama Mata Kuliah, Nilai Angka, Nilai Huruf
...
Jika kita menggunakan stringsAsFactors = FALSE, R akan memastikan bahwa kolom-kolom string tetap dalam tipe data character dan tidak diubah menjadi factor
