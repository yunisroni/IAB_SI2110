...
dplyr adalah library yang digunakan untuk manipulasi data: untuk melihat struktur data, menfilter data, men-select data, mengganti type data, dll)
...
```{r}
# untuk melihat parameter statistik dalam data
summary(asuransi)
```
...
```{r}
# your code
min(asuransi$children)
max(asuransi$children)
...
```{r}
# your code
rata_children <- mean(asuransi$children)
rata_children

```
```{r}
mean(asuransi$charges)
```

```{r}
mean_bmi <- mean(asuransi$bmi)
mean_bmi
```
...
```{r}
# your code
quantile(asuransi$children, 0.75)

```

```{r}
quantile(asuransi$children, 0.5)
```

```{r}
quantile(asuransi$children, 0.25)
```

```{r}
boxplot(asuransi$children)
```
...
```
#Latihan
Lakukan secara mandiri untuk variabel numerik yang lain dengan boxplot()
...
```{r}
# your code
sd(asuransi$children)
var(asuransi$children)
...
```{r}
# nilai frekuensi hubungan antara sex dengan region
...
```{r}
round(prop.table(table(asuransi$sex, asuransi$smoker),1),3)
```
```{r}
round(prop.table(table(asuransi$sex, asuransi$smoker),2),2)
```
